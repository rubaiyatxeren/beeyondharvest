...
import { DeliveryChargesModal } from './components/modals/DeliveryChargesModal';
import { ReviewModal } from './components/modals/ReviewModal';
import { CartProvider } from './components/providers/CartProvider';
import { NavProvider, useNav } from './components/providers/NavProvider';
import { ToastProvider } from './components/providers/ToastProvider';
import BeeTransferPage from './pages/BeeTransferPage';
...
// Inner component that has access to NavProvider
function AppContent() {
    ...
    useEffect(() => {
        ...
        if (review) {
            setReviewToken(review);
            const url = new URL(window.location.href);
            url.searchParams.delete("review");
            url.searchParams.delete("token");
            window.history.replaceState({}, "", url.toString());
        }
        if (p === "product" && id) setPage(`product:${id}`);
        else if (p === "complaint" && ticket) setPage("complaint");
        else if (p && p !== page) setPage(p);
        
        const shown = sessionStorage.getItem("delivery_modal_shown");
        if (!shown) {
            setTimeout(() => {
                setShowDeliveryModal(true);
                sessionStorage.setItem("delivery_modal_shown", "1");
            }, 2500);
        }
    ...
    const renderPage = () => {
      // Check for blog detail view first (format: blog:blogId)
      if (page.startsWith("blog:")) {
        const blogId = page.split(":")[1];
        return <BlogDetailPage key={blogId} blogId={blogId} />;
      }
      
      if (productId) return <ProductPage key={productId} productId={productId} />;
      
      switch (page) {
        case "home": return <HomePage key="home" />;
        case "shop": return <ShopPage key="shop" />;
        case "cart": return <CartPage key="cart" />;
        case "checkout": return <CheckoutPage key="checkout" />;
        case "track": return <TrackPage key="track" />;
        case "blog": return <BlogPage key="blog" />;
        case "complaint": return <ComplaintPage key="complaint" />;
        case "transfer": return <BeeTransferPage key="transfer" />;
        default: return <HomePage key="home" />;
      }
    };
    

    // Tailwind and Fonts Loader
...
//   // ✅ FIX: Round to 2 decimal places and ensure integer conversion where needed
...
//     return Math.round(disc * 100) / 100; // Round to 2 decimals
//   }, [coupon, subtotal]);
...
//   // ✅ FIX: Ensure total is properly rounded and can be integer if needed
//   const total = useMemo(() => {
//     const rawTotal = subtotal - discount + deliveryCharge;
//     // Round to 2 decimal places
//     const rounded = Math.round(rawTotal * 100) / 100;
//     // For display as integer if no decimals, but keep as number
//     return rounded;
...
//       const discountAmount = data.data?.discount || 0;           // ← calculated discount amount
//       const discountType = data.data?.discountType || "fixed";   // ← "percentage" or "fixed"
//       const discountValue = data.data?.discountValue || 0;       // ← the raw discount value

//       if (discountAmount > 0) {
//         cart.setCoupon({
//           code: couponCode,
//           type: discountType,
//           value: discountAmount,  // Store the calculated amount, not the raw value
//           rawValue: discountValue // Keep original for reference
//         });
...
// // ─── FIELD COMPONENT (move this OUTSIDE CheckoutPage) ────────────────────────
// const FormField = ({ label, k, type = "text", placeholder, options, required = false, half = false, value, error, onChange }) => (
...
//     canvas.width = 800;
//     canvas.height = 950;
//     // ... (keep your existing downloadSlip code)
...
//                 onChange={upd} placeholder="Rahim Hossain" required 
//               />
//               <FormField 
//                 label="Phone Number" k="phone" 
//                 value={form.phone} error={errors.phone}
//                 onChange={upd} placeholder="01XXXXXXXXX" required half 
//               />
//               <FormField 
//                 label="Email Address" k="email" type="email"
//                 value={form.email} error={errors.email}
//                 onChange={upd} placeholder="email@example.com" half 
//               />
//             </div>
//           </div>
//           <div className="bg-white rounded-2xl border border-stone-100 p-6">
...
//             <div className="flex flex-wrap gap-4">
//               <FormField 
//                 label="Street Address" k="street"
//                 value={form.street} error={errors.street}
//                 onChange={upd} placeholder="House #, Road #, Area" required 
//               />
//               <FormField 
//                 label="City/District" k="city"
//                 value={form.city} error={errors.city}
//                 onChange={upd} placeholder="Select your city" required options={cities} 
//               />
//               <FormField 
//                 label="Zip Code" k="zipCode"
//                 value={form.zipCode} error={errors.zipCode}
//                 onChange={upd} placeholder="e.g. 1200" required 
//               />
//             </div>
//           </div>
//           <div className="bg-white rounded-2xl border border-stone-100 p-6">
