/**
 * CartPage.jsx  (refactored)
 * ─────────────────────────────────────────────────────────────
 * Widgets consumed:
 *   DeliveryPicker  ← widgets/DeliveryBadge
 * ─────────────────────────────────────────────────────────────
 */
...
import { DeliveryPicker } from '../widgets/DeliveryBadge';
...
const CartPage = () => {
    ...
    const proceedToCheckout = () => {
        if (!selectedCity) { toast('Please select a delivery location', 'error'); return; }
        if (!cart.delivery) { toast('Please wait for delivery calculation', 'error'); return; }
        localStorage.setItem('bh_checkout_info', JSON.stringify({ checkoutCity: selectedCity, checkoutDelivery: cart.delivery }));
        setPage('checkout');
    };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-8 items-start">
                ...
                <div className="lg:sticky lg:top-20 space-y-4">
                    {/* ── Delivery picker widget ── */}
                    <div className="bg-white rounded-2xl border border-stone-100 p-5">
                        <DeliveryPicker
                            value={selectedCity}
                            onChange={setSelectedCity}
                            subtotal={cart.subtotal}
                            onDelivery={(data) => cart.setDelivery(data)}
                        />
                        {cart.delivery && (
                    ...
                    <div className="bg-white rounded-2xl border border-stone-100 p-5">
                        ...
                        <div className="space-y-3 text-sm">
                            ...
                            {cart.discount > 0 && (
                                <div className="flex justify-between text-emerald-600"><span>Discount</span><span className="font-semibold">-{fmt(cart.discount)}</span></div>
                            )}
                        ...
                        <button onClick={proceedToCheckout} disabled={!selectedCity || !cart.delivery}
